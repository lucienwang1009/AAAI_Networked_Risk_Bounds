...
Every vertex of $G$ is an object and described by a feature vector $X\in \xspace{}$ that is drawn independently and identically (i.i.d.) from an unknown distribution, while every edge corresponds to a training example whose input is a pair of feature vectors $\pairtwo{X,X'}$ of the two ends of this edge and whose target value $Y$ is in $\{0,1\}$.
% The classifier takes a pair of feature vectors $\pairtwo{X,X'}$ as input to predict the target value $Y\in \set{0,1}$. 
...
Each pair of vertices $\pairtwo{i,j}\in E$ corresponds to a networked example whose \emph{input} is a pair $(X_i,X_j)$ and \emph{target} value is $Y_{i,j}\in\yspace{}.$ 
We focus on \emph{binary classification} in this paper, i.e., $\yspace{} = \{0,1\}$. 
Moreover, the distribution of target values only depends on the features of the vertices it contains but does not depend on features of other vertices, that is, there is a probability distribution $P_{\yspace{}\mid\xspace{}^2}$ such that for every pair $\pairtwo{i,j}\in E$, the conditional probability 
...
The training set $\trainingset:=\{(X_i,X_j,Y_{i,j})\}_{\pairtwo{i,j}\in E}$ is \emph{dependent}\ copies of a generic random vector $(X_1, X_2, Y_{1,2})$ whose distribution $\distribution{}=P_\xspace\otimes P_\xspace\otimes P_{\yspace{}\mid\xspace{}^2}$ is fully determined by the pair $(P_\xspace{}, P_{\yspace{}\mid\xspace{}^2})$. 
...
ERM aims to find the function from a hypothesis set that minimizes the empirical estimator of \eqref{eq:graph_reconstruction_risk} on the training examples $\trainingset{}=\{(X_i,X_j,Y_{i,j})\}_{\pairtwo{i,j}\in E}$:
\begin{equation}
    \label{eq:graph_reconstruction_non_weighted_erm}
    \empiricalrisk{m}(r) := \frac{1}{m}\sum_{\pairtwo{i,j}\in E} \lossf(r, (X_i,X_j, Y_{i,j})).
\end{equation}
...
\begin{equation}
    ...
    \empiricalrisk{\weight}(r) := \frac{1}{\normo{\weight}} \sum_{\pairtwo{i,,j}\in E} w_{i,j} \lossf(r, (X_i,X_j, Y_{i,j}))
\end{equation}
...
vector $(w_{i,j})_{\pairtwo{i,j}\in E}$ that for every vertex $i\in V, \sum_{j:\pairtwo{i,j}\in E}w_{i,j}\le 1$.
\end{definition}
...
If for all $\pairtwo{i,j}\in E$, $w_{i,j}=1/|E|$, weighted ERM is degenerated to the classical ERM.
Let $E_<=\{\pairtwo{i,j}: i<j, \pairtwo{i,j}\in E\}$ be the ordered edge set. If Condition~\ref{con:symmetric} is satisfied, we also have
\begin{equation}
    \label{eq:symmetric_graph_reconstruction_imcomplete_erm}
    \empiricalrisk{\weight{}}(r) = \frac{2}{\normo{\weight{}}} \sum_{\pairtwo{i,j}\in E_<} w_{i,j} \lossf(r, (X_i,X_j, Y_{i,j})).
\end{equation}
\LongVersionEnd
...
\begin{theorem}[risk bounds]
    ...
    \begin{equation}
        \begin{aligned}
            \label{eq:main_result}
            \risk(r_\weight{}&)-\bayeserror \le 2(\inf_{r\in R}\risk(r)-\bayeserror) + \frac{K'C\log(1/\delta)}{(1-\complexbound)^{2/(2+\noise(\complexbound-1))}\normo{\weight{}}}\\
            &\bigg(\normo{\weight{}}^{(1-1/2+\noise(\complexbound-1))}+\max\Big(\|\weight{}\|_2,\|\weight{}\|_{\max}(\log(1/\delta))^{1/2},\\
            &\qquad\qquad\qquad\qquad\qquad\|\weight{}\|_\infty(\log(1/\delta))\Big)\bigg)
        \end{aligned}
    \end{equation}
    }
    ...
    \end{equation}
    \SingleColumnEnd
    where $\|\weight{}\|_{\max}=\max_i \sqrt{\sum_{j:\pairtwo{i,j}\in E} w_{i,j}^2}$ and $K'=\max(K,\sqrt{K},K^{1/(1+\complexbound{})})$.
\end{theorem}
...
\begin{align*}
    ...
    &=\frac{1}{\normo{\weight{}}}\sum_{\pairtwo{i,j}\in E} w_{i,j} q_r(X_i,X_j,Y_{i,j}).
\end{align*}
\DoubleColumnEnd
\SingleColumn
\begin{align*}
...
and
\[\widetilde{U}_\weight{}(r)=\frac{1}{\normo{\weight{}}}\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})\]
...
\begin{lemma}[condition leads to ``low-noise'', {\cite[Lemma 2]{papa2016graph}}]
    ...
    \begin{equation}
        \label{eq:variant_control}
        \Var\left[\E[q_r(X_1,X_2,Y_{1,2})\mid X_1]\right]\le c\Lambda(r)^\noise
    \end{equation}
    }
\end{lemma}
...
\begin{aligned}
    ...
    \mbox{s.t.} \quad& \forall \pairtwo{i,j}\in E, w_{i,j}\ge 0 \quad \mbox{and} \quad  \forall i, \sum_{j:\pairtwo{i,j}\in E} w_{i,j}\le 1
\end{aligned}
\end{equation}
\DoubleColumnEnd
\SingleColumn
...
$\normo{\weight{}} \le 1/\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j}.$
% \[\normo{\weight{}} \le \frac{1}{\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j}}.\]
Every distribution $\probdistri{}$ corresponds to a valid weighting vector $\weight{}$. 
% Since there exists at least one vertex $i$ such that $\sum_{j:\pairtwo{i,j}\in E} w_{i,j}= 1$ in the optimal solution of \eqref{eq:optimization_program}, 
...
%     (\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j})^{1/(1+\complexbound{})}+\max\Big(\|\probdistri{}\|_2,\\
%     &\qquad\|\probdistri{}\|_{\max}(\log(1/\delta))^{1/2},
%     \|\probdistri{}\|_\infty(\log(1/\delta))\Big)\\
%     \mbox{s.t.} \quad& \forall \pairtwo{i,j}\in E, p_{i,j}\ge 0 \quad \mbox{and} \quad \sum_{\pairtwo{i,j}\in E} p_{i,j}= 1
% \end{aligned}
% \end{equation}
% \DoubleColumnEnd
\SingleColumn
...
\begin{aligned}
    \min_{\probdistri{}} &\quad
    (\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j})^{1/(1+\complexbound{})}+\max\Big(\|\probdistri{}\|_2,\|\probdistri{}\|_{\max}(\log(1/\delta))^{1/2},
    \|\probdistri{}\|_\infty(\log(1/\delta))\Big)\\
    \mbox{s.t.} &\quad \forall \pairtwo{i,j}\in E, p_{i,j}\ge 0 \quad \mbox{and} \quad \sum_{\pairtwo{i,j}\in E} p_{i,j}= 1
\end{aligned}
\end{equation}
\SingleColumnEnd
% Note that $\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j}$ is the fractional matching number of $G$. 
...
\begin{aligned}
    ...
    \mbox{s.t.} \quad &\forall \pairtwo{i,j}\in E, p_{i,j}\ge 0; \quad \forall \pairtwo{i,j}\in E, p_{i,j}\log(1/\delta) - b \le 0\\
    & \forall i, \sum_{j:\pairtwo{i,j}\in E} p_{i,j} -a  \le 0; \quad \forall i, \left(\sum_{j:\pairtwo{i,j}\in E} p^2_{i,j}\log(1/\delta)\right)^{1/2} -b  \le 0\\
    & \|\probdistri{}\|_2 -b \le 0 \quad \mbox{and} \quad \sum_{\pairtwo{i,j}\in E} p_{i,j}= 1
\end{aligned}
\end{equation}
\SingleColumnEnd
\DoubleColumn
...
\begin{aligned}
    \min_{a,b,\probdistri{}} \quad&
    a^{1/(1+\complexbound{})}+b\\
    \mbox{s.t.} \quad &\forall \pairtwo{i,j}\in E, p_{i,j}\ge 0 \\
    & \forall \pairtwo{i,j}\in E, p_{i,j}\log(1/\delta) - b \le 0\\
    & \forall i, \sum_{j:\pairtwo{i,j}\in E} p_{i,j} -a  \le 0\\
    & \forall i, \left(\sum_{j:\pairtwo{i,j}\in E} p^2_{i,j}\log(1/\delta)\right)^{1/2} -b  \le 0\\
    & \|\probdistri{}\|_2 -b \le 0 \quad \mbox{and} \quad \sum_{\pairtwo{i,j}\in E} p_{i,j}= 1
\end{aligned}
\end{equation}
\DoubleColumnEnd
...
\begin{aligned}
    ...
    \mbox{s.t.} \quad &\forall \pairtwo{i,j}\in E, p_{i,j}\ge 0 \\
    & \forall i, \sum_{j:\pairtwo{i,j}\in E} p_{i,j} -a  \le 0\\
    & \sum_{\pairtwo{i,j}\in E} p_{i,j}= 1
\end{aligned}
\end{equation}
...
When graph $G$ is complete, weighting all examples equally gives the best risk bound, as all the terms $\max_{i=1,\dots,n} \sum_{j:\pairtwo{i,j}\in E} p_{i,j}$, $\|\probdistri{}\|_2$, $\|\probdistri{}\|_\max$ and $\|\probdistri{}\|_\infty$ achieve minimum. 
...
Let us discuss further the equal weighting scheme that gives every example the same weight. Denote by $\Delta(G)$ the maximum degree of $G$ (note that this is not the maximum degree of $D_G$) and let $p_{i,j}=1/m$ (recall that $m$ is the number of examples) for all $\pairtwo{i,j}\in E$. According to program \eqref{eq:optimization_program_final},
...
\begin{lemma}[decoupling and undecoupling]
    ...
    \begin{equation}
        ...
        \label{eq:weighted_u_statistics_decoupling}
            \E[\sup_{f_{i,j}\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in E}w_{i,j} f_{i,j}(X_i,X_j)|^q] \\
            &\le 4^q\E[\sup_{f_{i,j}\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E} w_{i,j}f_{i,j}(X_i,X_j^\prime)|^q]
        \end{aligned}
    \end{equation}
    \DoubleColumnEnd
    \SingleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistics_decoupling}
            \E[\sup_{f_{i,j}\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in E}w_{i,j} f_{i,j}(X_i,X_j)|^q] \le 4^q\E[\sup_{f_{i,j}\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E} w_{i,j}f_{i,j}(X_i,X_j^\prime)|^q]
        \end{aligned}
    \end{equation}
    \SingleColumnEnd
    If the functions $f_{i,j}$ are symmetric in the sense that for all $X_i,X_j$,
    \[f_{i,j}(X_i,X_j)=f_{j,i}(X_j,X_i)\]
    and $(w_{i,j})_{\pairtwo{i,j}\in E}$ is symmetric, then the inequality can be reversed, that is,
    \DoubleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistic_coupling}
        \E[\sup_{f_{i,j}\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in\E}w_{i,j} f_{i,j}(X_i,X_j^\prime)|^q]\\
        &\le 4^q \E[\sup_{f_{i,j}\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E}w_{i,j} f_{i,j}(X_i,X_j)|^q]
        \end{aligned}
    \end{equation}
    \DoubleColumnEnd
    \SingleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistic_coupling}
        \E[\sup_{f_{i,j}\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in\E}w_{i,j} f_{i,j}(X_i,X_j^\prime)|^q] \le 4^q \E[\sup_{f_{i,j}\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E}w_{i,j} f_{i,j}(X_i,X_j)|^q]
        \end{aligned}
    \end{equation}
    \SingleColumnEnd
\end{lemma}
\begin{lemma}[randomization]
    ...
    \DoubleColumn
    \begin{align*}
        \E[\sup_{f\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q]\\
        &\le 4^q\E[\sup_{f\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E}\sigma_i\sigma_j^\prime w_{i,j}f(X_i, X_j^\prime)|^q]
    \end{align*}
    \DoubleColumnEnd
    \SingleColumn
    \begin{align*}
        \E[\sup_{f\in\mathcal{F}}&|\sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q]\le 4^q\E[\sup_{f\in\mathcal{F}}|\sum_{\pairtwo{i,j}\in E}\sigma_i\sigma_j^\prime w_{i,j}f(X_i, X_j^\prime)|^q]
    \end{align*}
    \SingleColumnEnd
\end{lemma}
\begin{lemma}
    \label{le:weighted_decoupling_undecoupling}
    Let $(X_i^\prime)_{i=1}^n$ be an independent copy of the sequence $(X_i)_{i=1}^n$. Consider random variables valued in $\{0,1\}$, $(\tilde{Y}_{i,j})_{\pairtwo{i,j}\in E}$, conditionally independent given the $X_i^\prime$'s and the $X_i$'s and such that $\Pro[\tilde{Y}_{i,j}=1\mid X_i,X_j^\prime]=\eta(X_i, X_j^\prime)$. We have for all $q\ge 1$,
    \DoubleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistics_decoupling}
            \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j, Y_{i,j})|^q] \\
            &\le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q]
        \end{aligned}
    \end{equation}
    \DoubleColumnEnd
    \SingleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistics_decoupling}
            \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j, Y_{i,j})|^q] \le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q]
        \end{aligned}
    \end{equation}
    \SingleColumnEnd
    ...
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistic_coupling}
        \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in\E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q]\\
        &\le 4^q \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j,Y_{i,j})|^q]
        \end{aligned}
    \end{equation}
    \DoubleColumnEnd
    \SingleColumn
    \begin{equation}
        \begin{aligned}
        \label{eq:weighted_u_statistic_coupling}
        \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in\E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q] \le 4^q \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j,Y_{i,j})|^q]
        \end{aligned}
    \end{equation}
    \SingleColumnEnd
\end{lemma}
...
\begin{lemma}
    ...
    \begin{align*}
        \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q]\\
        &\le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}\sigma_i\sigma_j^\prime w_{i,j}\tilde{h}_r(X_i, X_j^\prime,\tilde{Y}_{i,j})|^q]
    \end{align*}
    \DoubleColumnEnd
    \SingleColumn
    \begin{align*}
        \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime, \tilde{Y}_{i,j})|^q] \le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}\sigma_i\sigma_j^\prime w_{i,j}\tilde{h}_r(X_i, X_j^\prime,\tilde{Y}_{i,j})|^q]
    \end{align*}
    \SingleColumnEnd
\end{lemma}
...
\begin{theorem}[moment inequality]
    ...
    \[Z = \sup_{f\in\mathcal F}|\sum_{\pairtwo{i,j}\in E}w_{i,j}f(X_i,X_j)|\]
    ...
    \[Z_\rademacher = \sup_{f\in\mathcal F}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\rademacher_i\rademacher_jf(X_i,X_j)|\]
    \[U_\rademacher = \sup_{f\in\mathcal F}\sup_{\alpha:\|\alpha\|_2\le 1}\sum_{\pairtwo{i,j}\in\E}w_{i,j}\rademacher_i\alpha_jf(X_i,X_j)\]
    ...
    % \SingleColumnEnd
    where $\|\weight{}\|_{\max}=\max_i \sqrt{\sum_{j:\pairtwo{i,j}\in E} w_{i,j}^2}$.
\end{theorem}
...
% \begin{align*}
%     &\E[\sup_f|\sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q]\\
%     &(f \text{ is degenerated})\\
%     &= \E[\sup_f|\sum_{\pairtwo{i,j}\in E} w_{i,j}(f(X_i,X_j^\prime)-\E_{\mathcal F^{\prime\prime}}[f(X_i,X_j^{\prime\prime})])|^q]\\
%     &(\text{Jansen's Inequality})\\
%     &\le \E[\sup_f|\sum_{\pairtwo{i,j}\in E} w_{i,j}(f(X_i,X_j^\prime)-f(X_i,X_j^{\prime\prime}))|^q]\\
%     &= \E[\sup_f|\sum_{j=1}^n \sum_{\pairtwo{i,j}\in E} w_{i,j}(f(X_i,X_j^\prime)-f(X_i,X_j^{\prime\prime}))|^q]
% \end{align*}
...
% \begin{align*}
%     \E[&\sup_f|\sum_{j=1}^n \sum_{\pairtwo{i,j}\in E} w_{i,j}(f(X_i,X_j^\prime)-f(X_i,X_j^{\prime\prime}))|^q\mid \mathcal F]\\
%     &= \E[\sup_f|\sum_{j=1}^n \sigma_j \sum_{\pairtwo{i,j}\in E} w_{i,j}(f(X_i,X_j^\prime)\\
%     &\qquad-f(X_i,X_j^{\prime\prime}))|^q\mid \mathcal F]\\
%     &(\text{Triangle's Inequality and the convexity of }\sup_f|\cdot|^q)\\
%     &\le 2^q\E[\sup_f|\sum_{j=1}^n \sigma_j \sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q\mid \mathcal F]\\
%     &\qquad+ 2^q\E[\sup_f|\sum_{j=1}^n \sigma_j \sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^{\prime\prime}))|^q\mid \mathcal F]\\
%     &\le 2^q\E[\sup_f|\sum_{j=1}^n\sigma_j\sum_{\pairtwo{i,j}\in E}w_{i,j} f(X_i,X_j^\prime)|^q\mid \mathcal F]
% \end{align*}
% and get
% \begin{align*}
%     \E[\sup_f&|\sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q] \\
%     &\le 2^q\E[\sup_f|\sum_{j=1}^n\sigma_j\sum_{\pairtwo{i,j}\in E}w_{i,j} f(X_i,X_j^\prime)|^q]
% \end{align*}
...
% \begin{align*}
%     \E[\sup_f&|\sum_{j=1}^n\sigma_j\sum_{\pairtwo{i,j}\in E}w_{i,j} f(X_i,X_j^\prime)|^q]\\
%     &\le \E[\sup_f|\sum_{i=1}^n \sum_{\pairtwo{i,j}\in E} w_{i,j}(\sigma_j f(X_i,X_j^\prime)\\
...
% \begin{align*}
%     \E[\sup_f&|\sum_{i=1}^n \sum_{\pairtwo{i,j}\in E} w_{i,j}(\sigma_j f(X_i,X_j^\prime)\\
%     &\qquad-\sigma_j f(X_i^{\prime\prime\prime},X_j^{\prime}))|^q\mid \mathcal F^\prime, (\sigma_i)_{i=1}^n]\\
%     &= \E[\sup_f|\sum_{i=1}^n \sigma_i^\prime \sum_{\pairtwo{i,j}\in E} w_{i,j}(\sigma_j f(X_i,X_j^\prime)-\\
%     &\qquad\sigma_j f(X_i^{\prime\prime\prime},X_j^{\prime}))|^q\mid \mathcal F^\prime, (\sigma_i)_{i=1}^n]\\
%     &\le 2^q\E[\sup_f|\sum_{i=1}^n \sigma_i^\prime \sum_{\pairtwo{i,j}\in E} w_{i,j}\sigma_j f(X_i,X_j^\prime)|^q\\
%     &\qquad\mid \mathcal F^\prime, (\sigma_i)_{i=1}^n]
% \end{align*}
% and get
% \begin{align*}
%     \E[\sup_f&|\sum_{j=1}^n\sigma_j\sum_{\pairtwo{i,j}\in E}w_{i,j} f(X_i,X_j^\prime)|^q]\\
%     &\le 2^q\E[\sup_f|\sum_{i=1}^n \sum_{\pairtwo{i,j}\in E} w_{i,j}\sigma_i^\prime \sigma_j f(X_i,X_j^\prime)|^q]
% \end{align*}
% Finally,
% \begin{align*}
%     \E[\sup_f&|\sum_{\pairtwo{i,j}\in E} w_{i,j}f(X_i,X_j^\prime)|^q]\\
%     &\le 4^q\E[\sup_f|\sum_{\pairtwo{i,j}\in E} w_{i,j}\sigma_i^\prime \sigma_j f(X_i,X_j^\prime)|^q].    
% \end{align*}
% \end{proof}
...
%     \begin{aligned}
%     L(a,b,&\probdistri{},\bmlambda) = a^{1/(1+r)} + b - \sum_{\pairtwo{i,j}\in E}\lambda^{(1)}_{i,j}p_{i,j} \\
%     &+ \sum_{\pairtwo{i,j}\in E}\lambda^{(2)}_{i,j}p_{i,j}\log(1/\delta)-\sum_{\pairtwo{i,j}\in E}\lambda^{(2)}_{i,j}b\\
%     &+ \sum_{i=1}^n \lambda^{(3)}_i\sum_{j:\pairtwo{i,j}\in E}p_{i,j}-\sum_{i=1}^n \lambda^{(3)}_i n_i a\\
...
%     &+v(\sum_{\pairtwo{i,j}\in E}p_{i,j}-1)-\lambda^{(6)}a-\lambda^{(7)}b
%     \end{aligned}
% \end{equation}
...
% \[1 - \sum_{\pairtwo{i,j}\in E}\lambda_{i,j}^{(2)} - \sum_{i=1}^n \lambda_i^{(4)}n_i - \lambda^{(5)} - \lambda^{(7)}=0\]
% and for all $\pairtwo{i,j}\in E$,
% \begin{align*}
%     -\lambda_{i,j}^{(1)} + \lambda_{i,j}^{(2)}\log(1/\delta) + \lambda_i^{(3)} + \lambda_i^{(4)}(\log(1/\delta))^{1/2}p_{i,j}\\(\sum_{k:\pairtwo{i,k}\in E}p_{i,k}^2)^{-1/2}+\lambda^{(5)}p_{i,j}\|\probdistri{}\|_2^{-1}+v=0.
% \end{align*}
...
%     &+b - b(\sum_{\pairtwo{i,j}\in E}\lambda_{i,j}^{(2)}+\sum_{i=1}^n\lambda_i^{(4)}n_i +\lambda^{(5)}+\lambda^{(7)})\\
%     &+ \|\probdistri{}\|_2\lambda^{(5)} + \sum_{i=1}^n\lambda_i^{(4)}(\sum_{j:\pairtwo{i,j}\in E}p_{i,j}^2)^{1/2}(\log(1/\delta))^{1/2}\\
%     &+\sum_{\pairtwo{i,j}\in E}p_{i,j}\Big(-\lambda_{i,j}^{(1)} + \lambda_{i,j}^{(2)}\log(1/\delta)+\lambda_i^{(3)}+v\Big)\\
%     &-v\\
%     & = \frac{r}{1+r}a^{1/(1+r)}+\sum_{i=1}^n\lambda_i^{(4)}(\sum_{j:\pairtwo{i,j}\in E}p_{i,j}^2)^{1/2}(\log(1/\delta))^{1/2}\\
%     &+\|\probdistri{}\|_2\lambda^{(5)} - \sum_{\pairtwo{i,j}\in E}p^2_{i,j}\Big(\lambda_i^{(4)}(\log(1/\delta))^{1/2}(\sum_{k:\pairtwo{i,k}\in E}p_{i,k}^2)^{-1/2}\\
%     &+\lambda^{(5)}\|\probdistri{}\|_2^{-1}\Big)+v\\
%     & = \frac{r}{(1+r)^{(1+2r)/r}}(\sum_{i=1}^n \lambda_i^{(3)}n_i + \lambda^{(8)})^{-1/r}\\
%     &+\sum_{i=1}^n\lambda_i^{(4)}(\sum_{j:\pairtwo{i,j}\in E}p_{i,j}^2)^{1/2}(\log(1/\delta))^{1/2}\\
%     &(1-\sum_{j:\pairtwo{i,j}\in E} p^2_{i,j}(\sum_{k:\pairtwo{i,k}\in E}p_{i,k}^2)^{-1})\\
%     &+\|\probdistri{}\|_2\lambda^{(5)}(1-\sum_{\pairtwo{i,j}\in E}p_{i,j}^2\|\probdistri{}\|_2^{-2})-v\\
...
% \[\sum_{\pairtwo{i,j}\in E}\lambda_{i,j}^{(2)} + \sum_{i=1}^n \lambda_i^{(4)}n_i + \lambda^{(5)} + \lambda^{(7)}=1,\]
...
%     &\sum_{\pairtwo{i,j}\in E}\lambda_{i,j}^{(2)} + \sum_{i=1}^n \lambda_i^{(4)}n_i + \lambda^{(5)} + \lambda^{(9)}=1\\
%     &\sum_{i=1}^n\lambda_i^{(3)}+\lambda^{(8)}=0\\
%     &\forall\pairtwo{i,j}\in E, -\lambda_{i,j}^{(1)}+\lambda_{i,j}^{(2)}\log(1/\delta)\\
...
\[4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}\sigma_i\sigma_j^\prime w_{i,j}\tilde{h}_r(X_i, X_j^\prime,\tilde{Y}_{i,j})|^q]\] 
...
\begin{proof}[Proof of Lemma~\ref{le:risk_bounds_iid}] % (fold)
    ...
    We denote $\{\bar{w}_i=\sum_{j:\pairtwo{i,j}\in E}w_{i,j}: i=1,\dots,n\}$ the weights on vertices and introduce the ``loss function''
    ...
    \todo{
    \begin{align*}
        d(r,r^*)\le \frac{\sqrt{\normo{\verticeweight{}}}}{\|\verticeweight\|_2}\sqrt{5c\relossf(r,r^*)^\noise}
    \end{align*}
    }
    ...
    \todo{
        \[\epsilon_*^2\le C^\prime K^{1/(2+\noise(\complexbound{}-1)}(\frac{1}{(1-\complexbound{})^2\normo{\verticeweight{}}})^{1/(2+\noise(\complexbound{}-1))}\]
    }
...
\begin{proof}[Proof of Lemma \ref{le:weighted_decoupling_undecoupling}] % (fold)
    ...
    \end{equation}
    Let $\set{\tilde{Y}_{i,j}}_{\pairtwo{i,j}\in E}$ be Bernoulli random variables such that $\Pro[\tilde{Y}_{i,j}=1 \mid \Sigma,\Sigma'] = \eta(X_i, X_j')$ and define for $\pairtwo{i,j}\in E$,
    \begin{equation*}
    ...
    \begin{align*}
        \E[&\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &=\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}4w_{i,j}\E[\tilde{h}_r(Z_i,Z_j^\prime,\tilde{Y}_{i,j})\mid\Sigma]|^q]\\
        &\le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(Z_i,Z_j^\prime,\tilde{Y}_{i,j})|^q]
    \end{align*}
    \DoubleColumnEnd
    \SingleColumn
    \begin{align*}
        \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]&=\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}4w_{i,j}\E[\tilde{h}_r(Z_i,Z_j^\prime,\hat{Y}_{i,j})\mid\Sigma]|^q]\\
        &\le 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(Z_i,Z_j^\prime,\hat{Y}_{i,j})|^q]
    \end{align*}
    \SingleColumnEnd
    ...
    \DoubleColumn
    \begin{align*}
        4^q\E[&\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\hat{h}_r(Z_i,Z_j^\prime,\tilde{Y}_{i,j})|^q]\\
        &= 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q]
    \end{align*}
    \DoubleColumnEnd
    \SingleColumn
    \begin{align*}
        4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(Z_i,Z_j^\prime,\hat{Y}_{i,j})|^q]= 4^q\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q]
    \end{align*}
    \SingleColumnEnd
    ...
    \DoubleColumn
    \begin{align*}
        \E[&\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j^\prime)|^q]\\
        &=\E[\sup_{r\in R} |\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})\\
        ...
        &\qquad-\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})\\
        &\qquad-\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &(\text{Triangle's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
        &\le \frac{1}{2}\E[\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})\\
        &\qquad+\tilde{h}_r(X_i^\prime,X_j,\tilde{Y}_{i,j})+\tilde{h}_r(X_i,X_j,Y_{i,j})\\
        &\qquad+\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &\qquad+ \frac{1}{4}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q] \\
        &\qquad+ \frac{1}{4}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &= \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\E_\sigma[\tilde{h}_r(Z_i,Z_j,\tilde{Y}_{i,j})]|^q]\\
        &\qquad+ \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &(\text{Jansen's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
        &\le \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(Z_i,Z_j,\tilde{Y}_{i,j})|^q] \\
        &\qquad+ \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &(\text{According to }\eqref{eq:weighted_u_statistics_decoupling_fact2})\\
        &= \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q] \\
        &\qquad+ \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        % &\le \E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &= 4^q\E[\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]
    \end{align*}
    \DoubleColumnEnd
    \SingleColumn
    \begin{align*}
        \E[&\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j^\prime)|^q]\\
        &=\E[\sup_{r\in R} |\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})+\tilde{h}_r(X_i^\prime,X_j,\tilde{Y}_{i,j}))|^q]\\
        &= \E[\sup_{r\in R} |\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})+\tilde{h}_r(X_i^\prime,X_j,\tilde{Y}_{i,j})+\tilde{h}_r(X_i,X_j,Y_{i,j})+\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})\\
        &\qquad-\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})-\frac{1}{2}\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &(\text{Triangle's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
        &\le \frac{1}{2}\E[\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})+\tilde{h}_r(X_i^\prime,X_j,\tilde{Y}_{i,j})+\tilde{h}_r(X_i,X_j,Y_{i,j})+\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &\qquad+\frac{1}{4}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q] + \frac{1}{4}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i^\prime,X_j^\prime,Y^\prime_{i,j})|^q]\\
        &= \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\E_\sigma[\tilde{h}_r(Z_i,Z_j,\tilde{Y}_{i,j})]|^q]+ \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &(\text{Jansen's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
        &\le \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(Z_i,Z_j,\tilde{Y}_{i,j})|^q] + \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &(\text{According to }\eqref{eq:weighted_u_statistics_decoupling_fact2})\\
        &= \frac{1}{2}\E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q] + \frac{1}{2}\E[\sup_{r\in R} |2\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &\le \E[\sup_{r\in R} |4\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]\\
        &= 4^q\E[\sup_{r\in R} |\sum_{\pairtwo{i,j}\in E}w_{i,j}\tilde{h}_r(X_i,X_j,Y_{i,j})|^q]
    \end{align*}
    \SingleColumnEnd
...
\begin{proof}[Proof of Lemma \ref{le:weighted_randomization}] % (fold)
    ...
    \DoubleColumn
\begin{align*}
    &\E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})|^q]\\
    &(\tilde{h}_r \text{ is degenerated})\\
    &= \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})\\
    &\qquad-\E_{\Sigma^{\prime\prime}}[\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime})])|^q]\\
    &(\text{Jansen's Inequality})\\
    &\le \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})\\
    &\qquad-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q]\\
    &= \E[\sup_{r\in R}|\sum_{j=1}^n \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})\\
    &\qquad-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q]
\end{align*}
\DoubleColumnEnd
    \SingleColumn
\begin{align*}
    \E[&\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})|^q]\\
    &(\tilde{h}_r \text{ is degenerated})\\
    &= \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})-\E_{\Sigma^{\prime\prime}}[\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime})])|^q]\\
    &(\text{Jansen's Inequality})\\
    &\le \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q]\\
    &= \E[\sup_{r\in R}|\sum_{j=1}^n \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j}^{\prime\prime})-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q]
\end{align*}
\SingleColumnEnd
...
\DoubleColumn
\begin{align*}
    \E[&\sup_{r\in R}|\sum_{j=1}^n \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})\\
    &\qquad-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &= \E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})\\
    &\qquad-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &(\text{Triangle's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
    &\le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q\mid \Sigma]\\
    &+ 2^q\E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &\le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n\sigma_j\sum_{i:\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q\mid \Sigma]
\end{align*}
\DoubleColumnEnd
\SingleColumn
\begin{align*}
    \E[&\sup_{r\in R}|\sum_{j=1}^n \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &= \E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}(\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})-\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &(\text{Triangle's Inequality and the convexity of }\sup_{r\in R}|\cdot|^q)\\
    &\le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q\mid \Sigma]\\
    &\qquad+ 2^q\E[\sup_{r\in R}|\sum_{j=1}^n \sigma_j \sum_{i:\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^{\prime\prime},\tilde{Y}_{i,j}^{\prime\prime}))|^q\mid \Sigma]\\
    &\le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n\sigma_j\sum_{i:\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q\mid \Sigma]
\end{align*}
\SingleColumnEnd
and get
...
\begin{align*}
    \E[\sup_{r\in R}&|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q] \\
    &\le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n\sigma_j\sum_{i:\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q]
\end{align*}
\DoubleColumnEnd
\SingleColumn
\begin{align*}
    \E[\sup_{r\in R}|\sum_{\pairtwo{i,j}\in E} w_{i,j}\tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q] \le 2^q\E[\sup_{r\in R}|\sum_{j=1}^n\sigma_j\sum_{i:\pairtwo{i,j}\in E}w_{i,j} \tilde{h}_r(X_i,X_j^\prime,\tilde{Y}_{i,j})|^q]
\end{align*}
\SingleColumnEnd
...
By the decoupling, undecoupling and randomization techniques (see Lemma~\ref{le:weighted_u_statistics_decoupling}, Lemma~\ref{le:weighted_u_statistics_randomization}), the symmetry and the degeneration of $f$ and the symmetry of $(w_{i,j})_{\pairtwo{i,j}\in E}$, we have
\begin{align*}
    \E[\sup_f&|\sum_{\pairtwo{i,j}\in E}w_{i,j} f(X_i,X_j)|^q]\\
    &\le 16^q\E[\sup_f|\sum_{\pairtwo{i,j}\in E}w_{i,j}\rademacher_i\rademacher_j^\prime f(X_i, X_j^\prime)|^q]\\
    &\le 64^q\E[\sup_f|\sum_{\pairtwo{i,j}\in E}w_{i,j}\rademacher_i\rademacher_j f(X_i, X_j)|^q]
\end{align*}
...
\[B=\sup_f\sup_{\alpha,\alpha^\prime:\|\alpha\|_2,\|\alpha^\prime\|_2\le 1}|\sum_{\pairtwo{i,j}\in E}w_{i,j}\alpha_i\alpha_j^\prime f(X_i,X_j)|.\]
...
which $Z_\rademacher^\prime = \sup_f|\sum_{\pairtwo{i,j}\in E}\rademacher_i\rademacher_j^\prime f(X_i,X_j)|$. Note that $\E_\rademacher$ now denotes expectation taken with respect to both the $\rademacher$ and the $\rademacher^\prime$. For simplicity, we denote by $A=\E_{\rademacher}[Z_\rademacher^\prime]$. In order to apply Corollary~$3$ of \cite{Boucheron2005}, define, for $k=1,\dots,n$, the random variables
\[A_k = \E_{\rademacher}[\sup_f |\sum_{\pairtwo{i,j}\in E,i,j\neq k}w_{i,j}\rademacher_i\rademacher_j^\prime f(X_i,X_j)|].\]
It is easy to see that $A_k\le A$.

On the other hand, defining
\[R_k=\sup_f|\sum_{i:\pairtwo{i,k}\in E}w_{i,k}\rademacher_if(X_i,X_k)|,\]
\[M=\max_kR_k\]
and denoting by $f^*$ the function achieving the maximum in the definition of $Z$, we clearly have
\begin{align*}
    A-A_k
    % &\le \E_\rademacher[\sum_{j:\pairtwo{k,j}\in E}w_{k,j}\rademacher_k\rademacher_j^\prime f^*(X_k,X_j)\\
    % &\qquad+\sum_{i:\pairtwo{i,k}\in E}w_{i,k}\rademacher_i\rademacher_k^\prime f^*(X_i,X_k)]\\
    &\le 2\E_\rademacher[M]
\end{align*}
and
\begin{align*}
    \sum_{k=1}^n(A-A_k)
    % &\le \E_\rademacher[\sum_{k=1}^n\rademacher_k\sum_{j:\pairtwo{k,j}\in E}w_{k,j}\rademacher_j^\prime f^*(X_k,X_j)\\
    % &\qquad+\sum_{k=1}^n\rademacher_k^\prime\sum_{i:\pairtwo{i,k}\in E}w_{i,k}\rademacher_i f^*(X_i,X_k)]\\
    &\le 2A.
\end{align*}
Therefore,
...
\[\sup_{f,i}\sup_{\alpha:\|\alpha\|_2\le 1}\sum_{j:\pairtwo{i,j}\in E}w_{i,j}\alpha_jf(X_i,X_j)\le \|\weight{}\|_{\max}F.\]
...
\textit{Third term:} $\E[B^q]$. By the Cauchy-Schwartz inequality, we have $B\le \sqrt{\sum_{\pairtwo{i,j}\in E}w_{i,j}^2}F= \|\weight{}\|_2F$ so
...
\[\{u_\rademacher(f) = \sum_{i=1}^n\rademacher_i\sum_{\pairtwo{i,j}\in\E}w_{i,j}\alpha_j^*\linebreak f(X_i,X_j),f\in\mathcal{F}\}.\]
...
%     &=(\E_\rademacher[|\sum_{\pairtwo{i,j}\in\E}w_{i,j}\rademacher_i\alpha_j^*(f(X_i,X_j)\\
%     &\qquad-g(X_i,X_j))|^2])^{1/2}\\
%     % &\le \sqrt{\sum_{\pairtwo{i,j}\in\E}(w_{i,j}\alpha_j^*)^2}\sup_{X_i,X_j\in\mathcal X^2}|f(X_i,X_j)\\
%     % &\qquad-g(X_i,X_j)|\\
...
% \SingleColumn
% \begin{align*}
%     (\E_\rademacher[|u_\rademacher(f)-u_\rademacher(g)|^2])^{1/2}&=(\E_\rademacher[|\sum_{\pairtwo{i,j}\in\E}w_{i,j}\rademacher_i\alpha_j^*(f(X_i,X_j)-g(X_i,X_j))|^2])^{1/2}\\
%     % &\le \sqrt{\sum_{\pairtwo{i,j}\in\E}(w_{i,j}\alpha_j^*)^2}\sup_{X_i,X_j\in\mathcal X^2}|f(X_i,X_j)\\
%     % &\qquad-g(X_i,X_j)|\\
%     &\le \|\weight{}\|_2\lnorm_\infty(f,g)
% \end{align*}
