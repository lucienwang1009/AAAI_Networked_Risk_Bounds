...
\begin{definition}[FPTAS]
  An algorithm $\mathcal{A}$ is a FPTAS for an optimization problem $\Pi$, if given an input $\mathcal{I}$ for $\Pi$ and $\epsilon>0$, $\mathcal{A}$ finds in time polynomial in both the size of $\mathcal{I}$ and $1/\epsilon$, a solution $s$ for $\mathcal{I}$ that satisfies
...
%   \todo{@Lucien: follow this idea: If $(a,b)$ is feasible for this program, then $(a',b)$ must be also feasible if $a'>a$.  }
  ...
  \end{equation}
  Finally, we select the approximate optimal weighting vector $\bar{\probdistri{}}$ from all possible solutions. 
  ...
  \end{align*}
  where $(a_{\bar{\probdistri{}}}, b_{\bar{\probdistri{}}}, \bar{\probdistri{}})\in Solutions$. 
\end{proof}
