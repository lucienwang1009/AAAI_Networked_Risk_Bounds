...
\end{aligned}
\end{equation}
\State Let $I:=\{\tilde{a}+i\cdot\epsilon(1+\beta)/n \mid  i\in \mathbb{N} \hbox{ and } i\le n(1-\tilde{a})/\epsilon(1+\beta)\}$. 
\For{$a \in I$}
...
\EndFor\\
\Return the best solution $\bar{\probdistri{}}$, $\bar{a}$ and $\bar{b}$ from $S$. 
\end{algorithmic}
\end{algorithm}
...
Because $a'\ge a^*$, $(a', b^*)$ is always a feasible solution for the program \eqref{eq:optimization_program_final_fix_a}, and thus we have $b'\le b^*$.
...
\end{equation}
Since in Algorithm \ref{alg:FPTAS} we select the optimal solution from all possible solutions $S$, combining \eqref{eq:a_mini_bound}, \eqref{eq:a_bound} and \eqref{eq:b_bound}, we have
\begin{align*}
