...
In this section, we use covering numbers as the complexity measurement of hypothesis sets, and then present our main results. 
...
Besides, \citet{Dudley1974Metric}, \citet{Korostelev1993Minimax} and \citet{Mammen1995Asymptotical} presented various examples of classes $\mathcal{F}$ satisfying Assumption \ref{ass:covering_number}.
...
Now we are ready to state the main result \todo{@Lucien: the algorithm is another main result} of the paper.
...
is a weighted \emph{degenerated} $U$-statistic \todo{@Lucien: if possible, explain what are degenerated U-statistics} of order $2$ with symmetric kernel 
...
with a \emph{degenerated} kernel \todo{@Lucien: if possible, explain what are degenerated kernels}
...
\begin{proof}[Proof of Theorem~\ref{th:main_theorem}]
    ...
    Let $A$ be the event on which\todo{@Lucien: I don't understand this sentence, in particular "on which"}
    ...
    \end{align*}
    \SingleColumnEnd
    for an appropriate constant $C_A$. Then by Lemma~\ref{le:uniform_approximation}, $\Pro[A]\ge 1-\delta/4$. Similarly, let $B$ be the event on which\todo{@Lucien: similar here}
    ...
    By \eqref{eq:graph_reconstruction_hoeffding_decomposition}, it is clear that, on $A\cap B$\todo{@Lucien: you want to say when both A and B heppen?}, $r_\weight{}$ is a $\rho$-minimizer of $T_\weight{}(r)$ over $r\in R$ in the sense that the value of this latter quantity at its minimum is at most $(\kappa_A+\kappa_B)$ smaller \todo than at $r_\weight{}$. 
    Then, from Lemma~\ref{le:risk_bounds_iid}, with probability at least $1-\delta/2$, $r_\weight{}$ is a $(\kappa_A+\kappa_B)$-minimizer of $T_\weight{}(r)$, which the result follows.\todo{@Lucien: we may need to go through this paragraph on Sunday}
\end{proof}
...
\section{Weight Vector Optimization} % (fold)
...
Then, we provide some methods that optimize the risk bounds progressively to find proper weights for general cases. \todo{@Lucien: maybe we now just mention this instead of presenting the details of this method, but need to point out we are solving a CONCAVE problem}
...
If the hypothesis set is a VC class or even finite \todo{@Lucien: we should be careful}, i.e., $\complexbound{}=0$, then the objective function becomes linear and \eqref{eq:optimization_program_final} is a convex optimization problem that can be solved by standard convex optimization methods (see e.g., \cite{boyd2004convex}) such as interior-point methods. 
...
\subsection{Fully polynomial-time approximation scheme} % (fold)
\label{sub:ptas}

In this section, we propose an algorithm to solve this problem and show that this algorithm is a fully polynomial-time approximation scheme. 
...
\State Let $I:=\{(1+i\cdot\epsilon(1+\beta))/n \mid  i\in \mathbb{N} \hbox{ and } i\le (n-1)/\epsilon(1+\beta)\}$. 
% \State Divide the domain of $a$ into intervals $Intervals$ with length $t=\epsilon(1+\beta)/n$;
...
\For{$a \in I$}
%   \State Fix $a=a'$;
  \State Solve the following program using the interior-point method with tolerance $\epsilon$ and add the solution into $S$.
  \begin{equation}
...
\EndFor\\
\Return the best weighting $\bar{\probdistri{}}$, $\bar{a}$ and $\bar{b}$ from $S$. 
\end{algorithmic}
\end{algorithm}

We prove that Algorithm \ref{alg:FPTAS} is a FPTAS. % for the program \eqref{eq:optimization_program_final}.
...
For any optimal solution $a^*$, we can find $a'$ in $Intervals$ such that $a^*\le a'<a^*+t$ and thus
\begin{equation}
\label{eq:a_bound}
\begin{aligned}
  (a')^{1/(1+\complexbound{})}&\le (a^*+t)^{1/(1+\complexbound{})}\\
  &\le (a^*)^{1/(1+\complexbound{})} + t*\frac{1}{1+\complexbound{}}(a^*)^{-\complexbound{}/(1+\complexbound{})}\\
  &\le (1+\epsilon)(a^*)^{1/(1+\complexbound{})}
\end{aligned}
\end{equation}
The third inequality follows from the fact that $1/n\le a$.
We assume that when we fix $a=a'$ the optimal solution for the program \eqref{eq:optimization_program_final} is $b=b'$.
Because $a'\ge a^*$, $(a', b^*)$ is always a feasible solution for the program \ref{eq:optimization_program_final}, and thus we have $b'\le b^*$.
Besides, the interior-point method should produce a $\epsilon$-approximate solution $b''$ such that
\begin{equation}
  \label{eq:b_bound}
  b''\le (1+\epsilon)b'\le (1+\epsilon)b^*.
\end{equation}
Since in Algorithm \ref{alg:FPTAS} we select the optimal solution from all $Intervals$, combining \eqref{eq:a_bound} and \eqref{eq:b_bound}, we have for the solution $(a', b'')$ such that $a^*\le a'<a^*+t$,
\begin{align*}
