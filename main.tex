...
If $\complexbound{}=0$, e.g., the hypothesis set is finite, then the objective function becomes linear and \eqref{eq:optimization_program_final} is a convex optimization problem that can be solved by standard convex optimization methods (see e.g., \cite{boyd2004convex}) such as interior-point methods. 
...
\end{aligned}
\end{equation}
\State Let $Grid:=\{\tilde{a}+i\cdot\epsilon(1+\beta)/n \mid  i\in \mathbb{N} \hbox{ and } i\le n(1-\tilde{a})/\epsilon(1+\beta)\}$ and $Solutions := \emptyset$. 
\For{$a \in Grid$}
...
Now we show that this algorithm indeed results in an $\epsilon$-approximation of this optimal solution. 
  ...
  For any optimal solution $a^*$ \todo{@Lucien: note that a solution should have three parts (a,b,p)}, if $a^*$ achieves minimum for the program \eqref{eq:optimization_program_final_fix_b}, we can find $a'$ in $Grid$ (actually $\tilde{a}$) such that 
  \begin{equation}
...
There is literature that uses Janson's decomposition derived from \cite{janson2004large} to study the problem learning from networked examples \cite{Usunier2005,Biau2006,ralaivola2009chromatic,DBLP:conf/icml/RalaivolaA15}.
...
\section{Proofs Omitted from the Main Track} % (fold)
...
\section{Proofs Omitted from the Appendix} % (fold)
