...
This optimization problem is concave, so we devise a new polynomial-time approximation scheme to solve it. 
\end{abstract}
...
However, not many theoretical guarantees of these methods have been established, which is the main concern of this paper. More specifically, this paper deals with risk bounds of the \emph{classifier trained with networked data (\problemabbr{})
% *** modify the name
...
Note that $1/n\le a\le 1$ in the program \eqref{eq:optimization_program_final}.
Using Algorithm \ref{alg:FPTAS}, we first use the interior-point method to find minimal feasible solution $\tilde{a}$ of the program \eqref{eq:optimization_program_final}. 
Since the interior-point method is FPTAS\cite{boyd2004convex}, solving this program needs polynomial time in the program size $m+n$ and $1/\epsilon$. 
Then the program \eqref{eq:optimization_program_final} is separated into
\[\frac{1-1/n}{\epsilon(1+\beta)/n}=\frac{n-1}{\epsilon(1+\complexbound{})}\]
convex programs.
% These convex programs can produce several $\epsilon$-approximate solutions by the interior-point method.
% Since the interior-point method is FPTAS\cite{boyd2004convex}, 
Solving each of these programs needs polynomial time in the problem size $m+n$ and $1/\epsilon$.
Thus, the complexity of Algorithm \ref{alg:FPTAS} is also polynomial in $m+n$ and $1/\epsilon$.

It is clear that the program \eqref{eq:optimization_program_final} always has the feasible solution, and thus the optimal solution $a^*$, $b^*$ and $\probdistri{}^*$.
For any optimal solution $a^*$, if $a^*$ achieve minimum for the program \eqref{eq:optimization_program_final_fix_b}, we can find $a'$ in $I$ (actually $\tilde{a}$) such that 
\begin{equation}
  \label{eq:a_mini_bound}
  (a')^{1/(1+\complexbound{})}\le (1+\epsilon)(a^*)^{1/(1+\complexbound{})}.
\end{equation}
Otherwise, we can also find $a'$ in $I$ such that $a^*\le a'<a^*+\epsilon(1+\beta)/n$ and thus
\begin{equation}
\label{eq:a_bound}
\begin{aligned}
