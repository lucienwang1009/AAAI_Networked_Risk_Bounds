...
\For{$a'$ in $Intervals$}
  \State Fix $a=a'$;
  \State Sovle the program \eqref{eq:optimization_program_final} by the interior-point method with tolerance $\epsilon$ and add the solution into $Solutions$.
\EndFor
\State Find the optimal weighting $\bar{\probdistri{}}$, $\bar{a}$ and $\bar{b}$ from $Solutions$;\\
\Return $\bar{\probdistri{}}$, $\bar{a}$ and $\bar{b}$.
\end{algorithmic}
\end{algorithm}
...
Using Algorithm \ref{alg:FPTAS}, we first separate the program \eqref{eq:optimization_program_final} into
...
Since the interior-point method is FPTAS\cite{boyd2004convex}, solving each of these programs needs polynomial time in the problem size $m+n$ and $1/\epsilon$.
