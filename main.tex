...
Most existing theoretical results of this problem only deal with the classical ERM\todo{full name?} principle that always weights every example equally, 
...
\textbf{Universal Risk Bounds}. We prove universal new risk bounds for \problemabbr{}. 
On the one hand, these bounds are more general, because they are proved for the situation that the graph is incomplete in \problemabbr{}. 
On the other hand, these bounds are based on weighted ERM and the ``low-noise'' condition, which makes them tighter than previous results on the incomplete graph.

\textbf{Optimization for Risk Bounds}. Since our new risk bounds depend on the weights added to every training examples, to achieve the tightest bound, we need to sovle a concave program for weighting vector.
Thus, we propose an algorithm to obtain the approximate optimal weighting vector and prove that the algorithm is a FPTAS for this concave program.
...
\todo{coherent}Compared with these results, the following three intuitions make our new risk bounds tighter and more general.
...
\For{$a \in Grid$}
  \State Use some interior-point methods to obtain an $\epsilon$-approximation of the following program and add the solution into $Solutions$.
  \begin{equation}
