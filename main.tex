...
\begin{table*}[!htbp]
  ...
  \begin{tabular}{M{2.7cm} M{2cm} M{4cm} M{3.7cm}}
    ...
    \cmidrule{1-4}
    \textbf{Weighted ERM} & \textbf{General graphs} & \cellcolor{Gray}\emph{This paper}  & \citec{ramon2015learning}{ALT}\\
    \bottomrule
  \end{tabular}
\end{table*}
...
\section{Weighting Optimization} % (fold)
...
Then, we provide some methods that optimize the risk bounds progressively %but may not find the global optimal solution 
to find proper weights for general cases. 
Using these methods, we are able to improve existing weighting schemes.
Finally, although the optimization problem is not convex, we prove that there exists a polynomial-time approximation scheme (PTAS) to solve this problem.
