...
Though this optimization problem is not convex, we devise a new polynomial-time approximation scheme to solve it. 
\end{abstract}
...
\todo{@Lucien and Yuyi: add something about the algorithm}
...
As mentioned by \citet{wang2017learning}, if several examples share a vertex, then we are likely to put less weight on them because the influence of this vertex to the empirical risk should be bounded. Otherwise, if we treat every example equally, then these dependent examples may dominate the training process and lead to the risk bounds that do not converge to $0$ (see the example in Section~\ref{sub:equally_weighting}).
...
Although this optimization problem is \emph{concave} unless the hypothesis set is finite, we present a \emph{fully polynomial-time approximation scheme (FPTAS)} to solve it.
...
\begin{definition}[FPTAS]
  ...
  \[|f_\Pi(s)-\mathrm{OPT}|\le \epsilon \cdot \mathrm{OPT},\]
...
If the hypothesis set is finite, e.g., $\complexbound{}=0$ and $K=|R|$, then the objective function becomes linear and thus \eqref{eq:optimization_program_final} is a convex optimization problem that can be solved by standard convex optimization methods (see e.g., \cite{boyd2004convex}) such as interior-point methods. 
...
Instead, one may not need to find the global optimum but only approximate it using some efficient methods, e.g., Concave-Convex Procedure \cite{Yuille2001The} and Coordinate Descent \cite{wright2015coordinate}.
...
% One of these methods is Concave-Convex Procedure \cite{Yuille2001The} which has been widely used in sparse support vector machine (SVM) and transductive SVM. 
...
\end{aligned}
\end{equation}
\State Let $Grid:=\{a_{min}+i\cdot\epsilon(1+\beta)/n \mid  i\in \mathbb{N} \hbox{ and } i\le n(1-\tilde{a})/\epsilon(1+\beta)\}$ and $Solutions := \emptyset$. 
\For{$a \in Grid$}
  \State Solve the following program using some interior-point method with tolerance $\epsilon$ and add the solution into $Solutions$.
  \begin{equation}
...
\EndFor\\
\Return the vector $\bar{\probdistri{}}$ which makes $\bar{a}^{1/(1+\beta)}+\bar{b}$ smallest from $Solutions$. 
...
We first analyze the running time of this algorithm.   
  ...
  convex programs, then each of which can produce an $\epsilon$-approximate solution by some interior-point methods. 
...
Now we show that this algorithm indeed results in an $\epsilon$-approximation of this optimal solution. 
  ...
  For any optimal solution $(a^*, b^*, \probdistri{}^*)$ \todo{@Lucien: note that a solution should have three parts (a,b,p)}, if $a^*$ achieves minimum for the program \todo{@Lucien: I am kind of confused here...} \eqref{eq:optimization_program_final_fix_b}, we can find $a'$ in $Grid$ (actually $a_{min}$) such that 
  \begin{equation}
...
%   \todo{@Lucien: follow this idea: If $(a,b)$ is feasible for this program, then $(a',b)$ must be also feasible if $a'>a$.  }
  ...
  \end{aligned}
  \end{equation}
  The third inequality follows from the fact that $1/n\le a^*$ \todo{@Lucien: and $1/n \le \tilde{a}$? }. 
  ...
  Besides, the interior-point method can produces an $\epsilon$-approximate solution $b''$ such that
  \begin{equation}
    \label{eq:b_bound}
...
However, as argued in Section~\ref{sub:intiution_and_examples} \todo{@Lucien: this ref}, one can construct examples to illustrate that if we use the equal weighting strategy when $\Delta(G)$ is large (e.g., if it is linear to $m$), the risk bounds \eqref{eq:equally_weighting_scheme} are very large and do not converge to $0$, while this problem can be solved by simply using a better weighting strategy. 
\begin{example}
...
Based on this result, one can train a classifier with better generalization by putting proper weights on networked examples. 
...
We also propose efficient optimization algorithms to improve the existing weighting schemes in general cases. 
...
This appendix is organized as follows. Section \ref{sec:fractional_coloring_approach} \todo{@Lucien: \ref{ } does not show, please check carefully for every place} provides new risk bounds for weighted ERM following the Janson's decomposition and show that it cannot improve prior results. 
...
\section{Proofs Omitted in Section \ref{sec:risk_bounds}} % (fold)
...
\section{Proofs Omitted in Section \ref{sec:fractional_coloring_approach}} % (fold)
...
\section{Proofs Omitted in Section \ref{sec:weighted_risk_bounds}} % (fold)
...
\section{Proofs Omitted in Section \ref{sec:a_moment_inequality_for_weighted_u_processes}} % (fold)
