...
\begin{proof}[Proof of Theorem~\ref{th:main_theorem}]
    ...
    Let $A$ be the event on which\todo{@Lucien: I don't understand this sentence, in particular "on which" (on the event)}
    ...
    $r_\weight{}$ is a $\rho$-minimizer of $T_\weight{}(r)$ over $r\in R$ in the sense that the value of this latter quantity at its minimum is at most $(\kappa_A+\kappa_B)$ smaller \todo than at $r_\weight{}$. 
...
Although this optimization problem is \emph{concave} unless the hypothesis set is finite, we present a fully polynomial-time approximation scheme (FPTAS) to solve this problem.
...
Instead, one may not need to find the global optimum and only approximate it using some efficient methods. 
