...
\pdfinfo{On the ERM Principle with Networked Data}
...
\begin{abstract}
Networked data, in which every training example involves two objects and multiple examples may share some common objects, is used in many machine learning tasks, e.g., learning to rank and link prediction.
...
However, not many theoretical guarantees of these methods have been established, which is the main concern of this paper. More specifically, this paper deals with risk bounds of \emph{classifiers trained with networked data (\problemabbr{})} whose goal is to train a classifier with examples in a data graph $G$. 
...
In order to improve this bound, \citet{ramon2015learning} propose \emph{weighted} ERM %\emph{(weighted ERM)}, 
...
However, \citet{ramon2015learning} (in fact, \citet{Usunier2005} and \citet{ralaivola2009chromatic} also) assume that any two examples can be arbitrarily correlated if they share a vertex, which cannot lead to an $O(\log(n)/n)$ bound when the graph is complete. 
...
\begin{table*}[!htbp]
  ...
  \begin{tabular}{M{2.7cm} M{2cm} M{4cm} M{3.7cm}}
    ...
    \cmidrule{1-4}
    \textbf{Weighted ERM} & \textbf{General graphs} & \cellcolor{Gray}\emph{This paper}  & \cite{ramon2015learning}\\
    \bottomrule
  \end{tabular}
\end{table*}

\section{Intuitions and Examples} % (fold)
...
If the graph is incomplete, one can use either Janson's decomposition \cite{janson2004large,Usunier2005,ralaivola2009chromatic,DBLP:conf/icml/RalaivolaA15} or the fractional matching approach by \citet{ramon2015learning} to derive risk bounds. 
The main differences between these two approaches are: 
\begin{itemize}
  \item \citet{ramon2015learning} considers the data graph $G$ while Janson's decomposition uses only the line graph $D_G$. 
...
\end{itemize}
Though \citet{ramon2015learning} show improved risk bounds, as far as we know, there is no known tight risk bounds on incomplete graphs for tasks such as pairwise ranking and graph reconstruction that satisfy the ``low-noise'' condition, under which the weighting scheme proposed by \citet{ramon2015learning} may be not good either (see Section~\ref{sub:complete_graph}).
...
%使用line graph会丢失信息
%Janson's decomposition似乎不太好，这个只是结果? 要不说Janson's Decomposition? 要不说line graph based method? 
...
%这个例子里，我们本来也就没法让它达到0,还是从monotonic来说？
% 但是weighted的bounds是趋近0的（随着m的增加），而unweighted不行
\paragraph{Asymptotic risk}
As mentioned by \citet{ramon2015learning}, if several examples share a vertex, then we are likely to put less weight on them because the influence of this vertex to the empirical risk should be bounded. Otherwise, if we treat every example equally, then these dependent examples may dominate the training process and lead to risk bounds that do not converge to $0$ (see the example in Section~\ref{sub:equally_weighting}).

\paragraph{Uniform bounds} 
Although \citet{DBLP:conf/icml/RalaivolaA15} prove an entropy-base concentration inequality for networked data using Janson's decomposition, the %``low-noise'' 
condition it required is too restrictive to be satisfied %in the case of complete graphs 
...
% Section~risk_bounds (end)
% In this section, we show that the risk bounds established by \citep{ramon2015learning} can be improved without any additional assumptions on the data distribution and the results obtained by \citep{papa2016graph} can be generalized into the case of incomplete graphs. 
...
$\mathcal{C}\subset \real{}^m$ is a $d_p$ cover of $\mathcal{F}$ on $x_1,\dots,x_m$ at scale $\epsilon>0$ if for all $f\in\mathcal{F}$, there exists $\mathbf{c}_f\in \mathcal{C}$ such that
$\|(f(x_1),\dots,f(x_m))-\mathbf{c}_f\|_p\le \epsilon.$
The empirical covering number $N_p(\mathcal{F},\epsilon;x_1,\dots,x_m)$ is the minimum cardinality of $\mathcal{C}$ such that $\mathcal{C}$ is a $d_p$ cover of $\mathcal{F}$ on $x_1,\dots,x_m$ at scale $\epsilon$.
We define the covering number  $N_p(\mathcal{F},\epsilon,m)=\sup_{x_1,\dots,x_m}N_p(\mathcal{F},\epsilon;x_1,\dots,x_m).$
We simply denote $N_p(\mathcal{F},\epsilon,m)$ by $N_p(\mathcal{F},\epsilon)$, if the context is clear.
\end{definition}
In this paper, we focus on the $\lebesgue_\infty$ covering number $N_\infty(\mathcal{F},\epsilon)$
and assume that it satisfies the following assumption.
...
We also refer to (\citeauthor{Mammen1998Smooth} \citeyear{Mammen1998Smooth}, p.\ 1813) for concrete examples of classes of the hypothesis sets with smooth boundaries satisfying Assumption~\ref{ass:covering_number}. 
...
\begin{theorem}[risk bounds]
    ...
    Consider a minimizer $r_\weight{}$ of the weighted empirical risk $\empiricalrisk{\weight{}}$ over a class $R$ that satisfies Assumption~\ref{ass:covering_number}. There exists a universal constant $C$ such that for all $\delta\in(0,1]$, with probability at least $1-\delta$, the excess risk of $r_\weight{}$ satisfies
    \DoubleColumn
    \begin{equation}
        \begin{aligned}
            \label{eq:main_result}
            \risk(r_\weight{}&)-\bayeserror \le 2(\inf_{r\in R}\risk(r)-\bayeserror) + \frac{KC\log(1/\delta)}{(1-\complexbound)^{2/(\complexbound+1)}\normo{\weight{}}}\\
...
In this case, our bounds are tighter than $O(1/\sqrt{\normo{\weight{}}})$ (recall that $\weight{}$ must be a fractional matching)
%是不是不要用1/\sqrt{\normo{\weight{}}}，因为1范数是可以变的，而p则更直接一些。但我不确定是在这里用个脚注还是换成p
% 这个我感觉还是挺显然的吧，用p就是不太好说
...
In the following, we bound these three terms $T_\weight{}, U_\weight{}$ and $\widetilde{U}_\weight{}$ respectively. 
...
By Lemma~\ref{le:variant_control}, $T_\weight{}(r)$ fulfills this condition.
...
\label{le:risk_bounds_iid}
  Suppose that $r'$ is a $\rho$-minimizer of $T_\weight{}(r)$ in the sense that $T_\weight{}(r')\le \rho + \inf_{r\in R}T_\weight{}(r)$ and $R$ satisfies Assumption~\ref{ass:covering_number}, then there exists a universal constant $C$ such that for all $\delta\in(0,1]$, with probability at least $1-\delta$, the risk of $r'$ satisfies
...
\section{Weighting Schemes} % (fold)
...
We show that, according to our bounds, equal weighting is indeed the optimal weighting scheme for complete graphs. 
We then discuss the performance of this equal weighting scheme when the graph is incomplete. % and show that it could provide a reasonable bounds. 
Finally, we provide some methods that optimize the risk bounds progressively %but may not find the global optimal solution 
to find the proper weight for general cases. 
Using these methods, we are able to improve existing weighting schemes. % and guarantee that the new scheme is not worse than the old one. 
...
Compared to the results in \cite{ramon2015learning}, our theory puts additional constrains on $\|\probdistri{}\|_2$, $\|\probdistri{}\|_\max$ and $\|\probdistri{}\|_\infty$ which encourages weighting examples fairly in this case, as illustrated in Figure~\ref{fig:weighting_scheme_complete_graph}. 
...
\begin{figure}[h]
  ...
  \DoubleColumn
    \subfigure[]{\label{fg:clique_1}
        \includegraphics[width=0.095\textwidth]{clique_1.eps}
    }\hspace{1ex}
    \subfigure[]{\label{fg:clique_2}
        \includegraphics[width=0.095\textwidth]{clique_2.eps}
    }\hspace{1ex}
    \subfigure[]{\label{fg:averaged_weighting}
        \includegraphics[width=0.095\textwidth]{complete_graph_1.eps}
    }\hspace{1ex}
    \subfigure[]{\label{fg:independent weighting}
        \includegraphics[width=0.095\textwidth]{complete_graph_2.eps}
    }
  \DoubleColumnEnd
  \SingleColumn
    \subfigure[First clique form]{\label{fg:clique_1}
        \includegraphics[width=0.17\textwidth]{clique_1.eps}
    }\hspace{5ex}
    \subfigure[Second clique form]{\label{fg:clique_2}
        \includegraphics[width=0.17\textwidth]{clique_2.eps}
    }\hspace{11ex}
    \subfigure[Equal weighting]{\label{fg:averaged_weighting}
        \includegraphics[width=0.17\textwidth]{complete_graph_1.eps}
    }\hspace{5ex}
    \subfigure[Independent subset]{\label{fg:independent weighting}
        \includegraphics[width=0.17\textwidth]{complete_graph_2.eps}
    }
  \SingleColumnEnd
...
%     \caption{Two weighting schemes for the complete graph formed by points $X_1$, $X_2$, $X_3$, $X_4$. Solid line means its weight $w>0$ while dash line means $w=0$. (a): Weights of every example equal $1/3$. (b): Only the examples in a independent set get weights $1$, i.e. $w_{1,4}=w_{4,1}=w_{2,3}=w_{3,2}=1$ and other weights are $0$. Note that $\normo{\probdistri{}}$ of these two weighting schemes are both $4$ but (a) have the tighter risk bounds from our results.}
...
% Next, we will introduce two approximation algorithms for \eqref{eq:optimization_program_final}.
% 证明coordinate descent是个PTAS的方法
...
% An advantage of these algorithms is that we can improve existing schemes for weighting networked examples, e.g., equal weighting, independent subset, maximum fractional matching or min-variance weighting \citep{ramon2015learning} by setting them as the initial solution (weighting scheme). 
